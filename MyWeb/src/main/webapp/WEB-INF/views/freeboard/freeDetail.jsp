            ...
            <div class="container">
                <div class="row">
                    <div class="col-xs-12 col-md-9 write-wrap">
                        ...
                        <div id="replyList">
                            <!-- 자바 스크립트르 활용하여 반복문을 이용해서 댓글의 개수만큼 반복 표현 
...
<script>
    window.onload =function(){
        ...
        document.getElementById('replyRegist').onclick=()=>{
            ...
            fetch('${pageContext.request.contextPath}/reply', reqObj)
                ...
                .then(data=>{
                    ...
                    //등록 성공 했다면, 다음 등록을 위해 입력창을 비워두자
                    document.getElementById('reply').value='';
                    document.getElementById('replyId').value='';
                    document.getElementById('replyPw').value='';
                    //등록 완료 후 댓글 목록 함수를 호출해서 비동기식을 목록 표현.
                    getList(1, true);
                });
        ...
        //게시글 상세보기 화면에 처음 진입했을 시 댓글 리스트를 한번에 불러오자
        getList(1,true);

        //댓글 목록을 가져올 함수
        //getList의 매개값으로 뭘 줄거냐?
        //요청된 페이지 번호와, 하면을 리셋할 것인지의 여부를 bool타입의 reset으로 받겠습니다.
        //비동기 방식이기 대문에 페이지가 그대로 계속 머물면서 댓글이 밑에 쌓입니다.
        //때에 따라서는 댓글을 계속 누적시키는 것이 아닌, 화면을 초기화 하고 새롭게 보여줘야 할 때가 있다.
        ...
        function getList(pageNum, reset) {
            ...
            .then(data =>{
                ...
                // 더보기 버튼 배치 판단
                // 현재 페이지 번호 * 이번 요청으로 받은 댓글수 > 전체 댓글수 ->더보기 없어도 됨.
                ...
                //중간중간에 들어가야할 글쓴이, 날짜,댓그 내용은 목록에서 꺼내서 표현.
                ...
                //id가 replyList 라는 div영역에 문자열 형식으로 모든댓글을 추가.
                //insertAdjacentHTML : 인접한 곳에 HTML 을 삽입하는 함수.
                //(position 문자열, 문자열 형태의HTML)
                if(!reset){
        ...
        function parseTime(regDate){
            ...
            //getTime() : 1970/1/1 자정을 기준으로 현재까지의 시간을 밀리초로 리턴.
            ...
            if(gap <60*60*24*1000){
                ...
                time = '\${regTime.getFullYear()}년 \${regTime.getMoth()+1}월 \${regTime.getDate()}일';
            }

...
//		System.out.println("fileName : "+fileName);
		...
		//응답에 대한 여러가지 정보를 전달 할 수 있는 객체 responseEntity
		...
		try {
			...
			//ResponseEntity 객체에 전달하고자 하는 파일을 byte[]로 변환해서 전달.(파일의 손상을 줋이기위해)
			//header 내용도 같이 포함, 응답 상태코드를 원하는 형태로 전달 가능.
			//생성자를 이용하여 responseEntity룰 생성하는 법.
			result = new ResponseEntity<>(FileCopyUtils.copyToByteArray(file),headers,HttpStatus.OK);
			
		} catch (Exception e) {
			e.printStackTrace();
			//static 메서드를 활용하여 responseEntity 객체를 생성하는 방법
	...
	//회원이 글 목ㄺ으로 진입 시 좋아요 게시물 리스트 체크

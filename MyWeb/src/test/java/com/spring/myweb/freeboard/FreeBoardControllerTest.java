...
public class FreeBoardControllerTest {
    ...
    */
	...
	@Autowired
	private WebApplicationContext ctx; // 컨테이너에 등록된 모든 빈을 사용할때.
	
//	@Autowired 
//	private FreeBoardController controller; -> 이 컨트롤러 하나만 쓸때.
...
//	@AfterEach//테스트 시작이후에
	public void setup() {
		...
		//테스트할 컨트롤러를 수종으로 주입. 하나의 컨트롤러 자체만 테스트를 진행할때는 이렇게 사용
//		this.mockMvc = MockMvcBuilders.standaloneSetup(controller).build();
	...
	@Test
	@DisplayName("게시글 등록 완료 후 처리 과정 테스트")
	...
	@Test
	@DisplayName("3번글 상세보기 요청을 넣으면 컨트롤러는 DB에서 가지고 온 글 객체를 model에 담아 freeDetail.jsp로 이동시킬 것이다.")
	void testContent() throws Exception {
		// /freeboard/content -> get
		// bno, title,writer,content,update==null? regDate, update(수정됨)

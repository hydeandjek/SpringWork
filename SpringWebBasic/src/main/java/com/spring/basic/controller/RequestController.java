...
public class RequestController {
	...
	/*
	 만약 사용자가 /request/req 요청을 보내왔을때 Views폴더 아래에
	 request폴더 안에 존재하는 req-ex01.jsp파일을 열도록 메서드를 구상해보세요.
	 */
...
//	@RequestMapping(value = "/request/basic01" , method = RequestMethod.POST)
	...
	//요청 URL 주소가 같더라도, 전송 방식에 ㄸ라 맵핑을 다르게 하기 때문에
	//같은 주소를 사용하는 것이 가능합니다.(GET -> 화면처리 , POST ->입력값처리)
	...
	파라미터 변수 명과 값을 받을 변수명을 동일하게 작성하면 @RequestParam생략가능.
	...
	*/
	...
	/*
	 3. 커맨드 객체를 활용한 파라미터 처리
	  ...
	  - VO클래스의 필드는 파라밑 변수명과 동일하게 작성합니다.(setter메서드를 호출)
